...
class ResursPipeline:

    # Database setup
    conn = sqlite3.connect('resurs.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `resurs`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM resurs WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO resurs (date, title, link, content)"
